...
#define DEF_MAX_OF_LOG_BUFFER 1024
static char 
  log_of_self_test[DEF_MAX_OF_LOG_BUFFER] = {0,};
...
int inv_device_init_log(void)
{
	memset(log_of_self_test, 0x0, DEF_MAX_OF_LOG_BUFFER);
	return 0;
}


int inv_device_put_log(const char* fmt, ...)
{
    int result;
    int len = 0, offset = 0;
	va_list args;
    ...
    len = strlen(log_of_self_test);
    if(len+strlen(buf) < DEF_MAX_OF_LOG_BUFFER) {
      offset = len;
      len = strlen(buf);
      memcpy(&log_of_self_test[offset], buf, len);
      offset += len;
      log_of_self_test[offset] = 0; 
    } else {
      MPL_LOGI("Error : out of memory for log");
...
const char* inv_device_get_log(void)
{
  return (const char*)log_of_self_test;
}
                  ...
                  uint_fast8_t perform_full_test)
{
    ...
    MPL_LOGI("bias          : %+13.3f %+13.3f %+13.3f (LSB)\n",
             avg[X], avg[Y], avg[Z]);
    inv_device_put_log("bias :%+.3f %+.3f %+.3f (dps)\n",
             avg[X]/adj_gyro_sens, avg[Y]/adj_gyro_sens, avg[Z]/adj_gyro_sens);
    if (VERBOSE_OUT) {
    ...
    for (j = 0; j < 3; j++) {
        if (fabs(avg[j]) > (float)test_setup.bias_thresh) {
            ...
            inv_device_put_log("%s-Gyro bias (%.0f) exceeded threshold "
                    "(threshold = %d LSB)\n",
                    a_name[j], avg[j], test_setup.bias_thresh);
            ret_val |= 1 << (3+j);
        }
    ...
    inv_device_put_log("RMS : %+.3f %+.3f %+.3f (dps-rms)\n",
             sqrt(rms[X]/total_count)/ adj_gyro_sens, 
             sqrt(rms[Y]/total_count)/ adj_gyro_sens, 
             sqrt(rms[Z]/total_count)/ adj_gyro_sens);
