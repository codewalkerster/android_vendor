        ...
        if (!mldl_cfg->slave[EXT_SLAVE_TYPE_COMPASS]) {
            ...
            if (h_akm_lib) {
                ...
                /* 
                    @date : 2011,10.25 
                    @name : jushin 
                    @comment : change h_dmp_lib to h_akm_lib  
                **/ 
...
/*
  @date : 2011.11.03 
  @description : applied the patch from "Jeff Lai" , because of gimbals lock problem   
**/
    //Azimuth
    #define XYZ	(0)
    #define YXZ	(1)
    ...
    eular[XYZ][Z_ANGLE] = atan2(R[3], R[0])*57.295779513082320876798154814105f;

    eular[YXZ][X_ANGLE] = asin(R[7])*57.295779513082320876798154814105f;
    eular[YXZ][Y_ANGLE] = atan2(-R[6], R[8])*57.295779513082320876798154814105f;
    eular[YXZ][Z_ANGLE] = atan2(-R[1], R[4])*57.295779513082320876798154814105f;

    float a = fabs(eular[YXZ][X_ANGLE]/90);
    a = a*a;

    float p = (fabs(eular[YXZ][Y_ANGLE])/90 - 1.0);
    if(p < 0 )
        p = 0;
    ...
    if( ( eular[YXZ][X_ANGLE] > 0 &&  eular[YXZ][Y_ANGLE] > 0 ) || ( eular[YXZ][X_ANGLE] < 0 &&	eular[YXZ][Y_ANGLE] < 0 ) )
    {
        if( eular[YXZ][Z_ANGLE] > 0 && eular[XYZ][Z_ANGLE] < 0 )
        {
            eular[XYZ][Z_ANGLE] +=360;
        }
    }
    else if( ( eular[YXZ][X_ANGLE] > 0 &&	eular[YXZ][Y_ANGLE] < 0 ) || ( eular[YXZ][X_ANGLE] < 0 &&  eular[YXZ][Y_ANGLE] > 0 ) )
    {
        if( eular[YXZ][Z_ANGLE] < 0 && eular[XYZ][Z_ANGLE] > 0 )
        {
            eular[XYZ][Z_ANGLE] -=360;
    ...
    values[0] = (1-a*v) * eular[YXZ][Z_ANGLE] + (a*v) * eular[XYZ][Z_ANGLE];
