...
void MPLSensorSysPed::setupPedFp()
{
    ...
    if (mCurFifoRate > 6) {
        ALOGV_IF(EXTRA_VERBOSE, "need to adjust fifo rate (cur = %d)", mCurFifoRate);
        result = inv_set_fifo_rate(6);
        ALOGE_IF(result != INV_SUCCESS, "setupPedFp : failed to adjust fifo rate");
        mCurFifoRate = 6;
    }
    ALOGV("fifo rate - divider : %d, delay : %llu ns", 
         mCurFifoRate, (unsigned long long)mCurFifoRate * 5000000LLU);

    result = inv_enable_full_power_pedometer();
    if (result != INV_SUCCESS) {
        ALOGE("Fatal error: inv_enable_full_power_pedometer returned %d\n", result);
    }

    result = inv_set_full_power_pedometer_params(&mStepParams);
    if (result != INV_SUCCESS) {
        ALOGE("Fatal error: inv_set_full_power_pedometer_params returned %d\n", result);
    }

    result = inv_set_full_power_pedometer_step_callback(step_cb_wrapper_fp);
    if (result != INV_SUCCESS) {
        ALOGE("Fatal error: inv_set_full_power_pedometer_step_callback returned %d\n", result);
    }

    result = inv_set_full_power_pedometer_step_count(mPedSteps);
    result = inv_set_full_power_pedometer_walk_time(mPedWt);

    ALOGE_IF(result != INV_SUCCESS, "Fatal error: inv_set_full_power_pedometer_step_count failed (%d)", result);
...
int MPLSensorSysPed::readEvents(sensors_event_t* data, int count)
{
    ...
    if (mDmpStarted) {
        ...
        if(mPedState == PED_SLEEP && irqSet[ACCELIRQ_FD]) {
            ...
            ALOGE_IF(e != INV_SUCCESS, "failed to reset step count in Standalone Pedometer on motion int");
            inv_set_low_power_pedometer_walk_time(mPedWt);
            e = inv_start_low_power_pedometer();
            ALOGE_IF(e != INV_SUCCESS, "failed to restart standalone pedometer on motion int");
            mPedState = PED_STANDALONE;
            ...
            ALOGE_IF(e != INV_SUCCESS, "failed to sleep pedometer on no-motion event");
            mPedState = PED_SLEEP;
        }
    } else {
...
int MPLSensorSysPed::rpcStopPed()
{
    ...
    if(mPedState == PED_SLEEP || mPedState == PED_STANDALONE) {
        ALOGV_IF(EXTRA_VERBOSE && mPedState == PED_SLEEP, "sys ped was asleep");
        inv_close_low_power_pedometer();
        mPedState = PED_NONE;
        mPedSteps = 0;
        mPedWt = 0;
