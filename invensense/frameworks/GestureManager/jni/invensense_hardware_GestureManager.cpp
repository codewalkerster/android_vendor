...
{
    sp<SensorEventQueue> queue(reinterpret_cast<SensorEventQueue *>(nativeQueue));
    ALOGD("gestures_enable_gesture %d %d %d", nativeQueue, gesture, delay );
...
/*
    ALOGE("Gesture Event");
    ALOGE("  ver: %d", event.version);
    ALOGE("  sen: %d", event.sensor);
    ALOGE("  typ: %d", event.type);
*/
...
sp<IMplConnection> get_mpl_binder() {
    ...
    if (s_mpl_b == NULL)
    {
      ALOGE("The mpld is not published");
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    if( jniRegisterNativeMethods(env, "com/invensense/android/hardware/glyphapiJNI", glyphMethods, NELEM(glyphMethods)) != 0) {
        ALOGE("ERROR: Could not register native methods for glyphapiJNI\n");
        goto bail;
    }
